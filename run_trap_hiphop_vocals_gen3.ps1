# ============================================================
#  Trap / Hip-Hop  VOCAL MASK  --  Generation 3  (1000-Track Payload)
#  500 Trap + 500 Hip-Hop  |  Top-5 seeds from Vocals Gen2, mutation 0.05
# ============================================================

$Root   = $PSScriptRoot
$Gen2   = Join-Path $Root "trap_hiphop_vocals_gen2"
$OutDir = Join-Path $Root "trap_hiphop_vocals_gen3"
$Python = "python"
...
if (Test-Path $OutDir) {
    Write-Host "Removing old trap_hiphop_vocals_gen3..." -ForegroundColor Yellow
...
# ---- Pure Math Grader -------------------------------------------------------
Write-Host ""
Write-Host "Running Pure Math Grader on Vocals Generation 3..." -ForegroundColor Magenta
& $Python -m src.orchestration.telemetry_grader_midi --batch-dir $OutDir

# ---- Cross-Generation Comparison --------------------------------------------
Write-Host ""
Write-Host "Cross-Generation Comparison (Vocal Mask pipeline)..." -ForegroundColor Magenta

$Batches = @(
    @{ path = (Join-Path $Root "trap_hiphop_vocals_gen1"); label = "Vocals Gen1 Baseline   (200 tracks)" },
    @{ path = (Join-Path $Root "trap_hiphop_vocals_gen2"); label = "Vocals Gen2 Evolved    (500 tracks)" },
    @{ path = (Join-Path $Root "trap_hiphop_vocals_gen3"); label = "Vocals Gen3 Commercial (1000 tracks)" }
)
...
Write-Host "1000-track vocal mask payload ready at:" -ForegroundColor Cyan
Write-Host "  $OutDir"

# ============================================================
#  EDM / House  --  Generation 2  (Golden Matrix Injection)
#  250 EDM + 250 House  |  Top-5 seeds from Gen1, mutation 0.05
# ============================================================

$Root   = $PSScriptRoot
$Gen1   = Join-Path $Root "edm_gen1"
$OutDir = Join-Path $Root "edm_gen2"
$Python = "python"
...
if (Test-Path $OutDir) {
    Write-Host "Removing old edm_gen2..." -ForegroundColor Yellow
...
# ---- Pure Math Grader -------------------------------------------------------
Write-Host ""
Write-Host "Running Pure Math Grader on EDM Generation 2..." -ForegroundColor Magenta
& $Python -m src.orchestration.telemetry_grader_midi --batch-dir $OutDir

$TotalTime = ((Get-Date) - $StartAll).ToString("hh\:mm\:ss")
Write-Host ""
Write-Host ("EDM Generation 2 complete.  Total time: $TotalTime") -ForegroundColor Green
...
Write-Host ""
Write-Host "Next step: run_edm_gen3.ps1" -ForegroundColor Yellow

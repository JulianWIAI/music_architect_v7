# ============================================================
#  Vocals Generation 2  -  100 tracks per genre
#  Seed pool: Top-5 Golden Matrices from vocals_generation_1
#  Mutation factor: 0.05  |  Vocal mask: ON
# ============================================================

$Root    = $PSScriptRoot
$VocGen1 = Join-Path $Root "vocals_generation_1"
$OutDir  = Join-Path $Root "vocals_generation_2"
$Python  = "python"
...
if (Test-Path $OutDir) {
    Write-Host "Removing old vocals_generation_2..." -ForegroundColor Yellow
...
foreach ($g in $Genres) {
    ...
    $SeedPool = Join-Path $VocGen1 ($g.genre + "\math_fitness_report.json")
    ...
    if (Test-Path $SeedPool) {
        ...
        Write-Host "  [WARN] No vocals_generation_1 report - free exploration" -ForegroundColor Yellow
    }
...
# ── Pure Math Grader ─────────────────────────────────────────
Write-Host ""
Write-Host "Running Pure Math Grader on vocals_generation_2..." -ForegroundColor Magenta

& $Python -m src.orchestration.telemetry_grader_midi --batch-dir $OutDir

$TotalTime = ((Get-Date) - $StartAll).ToString("hh\:mm\:ss")
Write-Host ""
Write-Host ("Vocals Generation 2 complete.  Total time: $TotalTime") -ForegroundColor Green
